var OSRM_PROVIDER_URL = "https://maps.sylphen.com/osrm"; // hackish replacement

/*
 leaflet-routing-machine.js,
 a JavaScript library for routing in leaflet maps: http://www.liedman.net/leaflet-routing-machine/
 (c) 18. Januar 2016: Modifications by  Josef Gräf, www.cursor.de
*/
...
(function() {
	...
	L.Routing.Control = L.Routing.Itinerary.extend({
		...
		_routeSelected: function(e) {
			...
			if (fitBounds) {
				this._map.fitBounds(this._line.getBounds()); //20160114jg Testweise auskommentiert
			}
				...
				this._router.route(wps, options.callback || function(err, routes) {
					...
					if (ts === this._requestCount) {
						...
						if (!options.geometryOnly) {
							...
							this.setAlternatives(routes);    //20160107jg: Fehlerrr!!! Bei OSRMTrip testweise auskommentiert
						} else {
...
(function() {
	...
	L.Routing.Line = L.LayerGroup.extend({
		...
		options: {
			...
			//20160114jg: Graue Luftlinien unsichtbar machen.
			// missingRouteStyles: [
			...
			missingRouteStyles: [
				...
				{color: 'white', opacity: 0.0, weight: 0},
				{color: 'gray', opacity: 0.0, weight: 0, dashArray: '7,12'}
			],
			addWaypoints: true,
...
(function() {
	...
	L.Routing.OSRM = L.Class.extend({
		options: {
			serviceUrl: OSRM_PROVIDER_URL + '/viaroute',
			timeout: 30 * 1000,
			routingOptions: {}
		},
		...
		route: function(waypoints, callback, context, options) {
			...
			corslite(url, L.bind(function(err, resp) {
				...
				if (!timedOut) {
					...
					if (!err) {
						try {
							data = JSON.parse(resp.responseText);   //20160106jg bis hier her
							try {
		...
		buildRouteUrl: function(waypoints, options) {
			...
			//20160107jg: Zur Kontrolle
			/*
            alert(this.options.serviceUrl + '?' +
...
(function() {
	...
	//20160114jg Experiment
			var LeafIcon = L.Icon.extend({
				options: {
					iconSize:     [12, 16],
					iconAnchor:   [6, 0],    //[6, 20] bei ungespiegelten Markern
					popupAnchor:  [0, -20]
				}
			});
			var redIcon = new LeafIcon({iconUrl: './img/marker-icon-red_dreieck.png'});
			var LeafIcon_startend = L.Icon.extend({
				options: {
					iconSize:     [17, 22],
					iconAnchor:   [6, 0],    //[6, 20] bei ungespiegelten Markern
					popupAnchor:  [0, -20]
			...
			var redIcon_startend = new LeafIcon_startend({iconUrl: './img/marker-icon-red_startend.png'});
			var LeafIcon_drag = L.Icon.extend({
				options: {
					iconSize:     [12, 20],
					iconAnchor:   [6, 20],
					popupAnchor:  [0, -20]
			...
			var redIcon_drag = new LeafIcon_drag({iconUrl: './img/marker-icon-red_drag.png'});
	//20160114jg Ende Experiment
	...
	L.Routing.Plan = L.Class.extend({
		...
		options: {
			...
			createMarker: function(i, wp) {
				...
				var options = {
						...
						icon: redIcon //20160114jg Experiment
					},
				    marker = L.marker(wp.latLng, options);
				} else {
				var options = {
						draggable: this.draggableWaypoints,
						icon: redIcon_startend //20160114jg Experiment
					},
				    marker = L.marker(wp.latLng, options);
				}
				return marker;
			...
			//20160118jg
			createMarker_drag: function(i, wp) {
				var options = {
						draggable: this.draggableWaypoints,
						icon: redIcon_drag
					},
		...
		_dragNewWaypoint: function(newWpIndex, initialLatLng) {
			var wp = new L.Routing.Waypoint(initialLatLng),
				...
				marker = this.options.createMarker_drag(newWpIndex, wp, this._waypoints.length + 1),
				lines = [],
				mouseMove = L.bind(function(e) {
					var i;
					if (marker) {
				...
				mouseUp = L.bind(function(e) {
					var i;
					if (marker) {
						this._map.removeLayer(marker);     //20160118jg: auskommentieren, wenn der drag-Marker soll nicht verschwinden soll.
					}
					for (i = 0; i < lines.length; i++) {
